  ...
  for (const key of PUSH_TEMPLATE_KEYS) {
    it(`${key}: 금지 표현·이모지 없음, 해요체, 길이 제한`, () => {
      for (const params of SAMPLES[key]) {
        ...
        // 사용자 대상 본문은 해요체("요.")·C1 확정 서브카피("… N건")·수신거부 문구 중 하나로 끝난다 (admin_alert 는 운영 요약 원문)
        if (key !== "admin_alert") expect(/(요\.?|요\)|해제|\d+건)$/.test(r.body) || r.body.length === 0, `${key} body: ${r.body}`).toBe(true);
