...
export type PromotionResult =
  ...
  | { ok: false; code: "PROFILE_NOT_FOUND" | "PHONE_BLOCKED" | "CI_BLOCKED" | "UNDERAGE" | "DB_ERROR"; message: string };
...
export async function promoteIdentityVerified(
  ...
  // 2) 만 19세 미만 → 즉시 banned + 파기 큐 등록 (A5 §1.3 게이트 2)
  ...
  // 3) 승급 + PASS 생년월일 덮어쓰기 (자기신고와 불일치해도 성인이면 PASS 값으로 계속)
  ...
  await writeAuditLog(profile.id, "verify.level2", `profile:${profile.id}`, {
    ci_hash: hashIdentity(input.ci), // 원문 미저장
