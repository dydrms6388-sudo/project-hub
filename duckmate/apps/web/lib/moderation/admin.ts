...
export async function adminModerationStats(): Promise<Record<string, Json>> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_moderation_stats", { p_actor_id: a })) as Record<string, Json>;
}
...
export async function adminListReports(filter: ReportListFilter = {}, cursor: ReportListCursor | null = null, limit = 30): Promise<ReportListResult> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_list_reports", { p_actor_id: a, p_filter: filter as Json, p_cursor: (cursor as Json) ?? null, p_limit: limit })) as unknown as ReportListResult;
}
...
export async function adminGetReport(reportId: string): Promise<ReportDetail> {
  ...
  const raw = (await moderationRpc(admin, "admin_get_report", { p_actor_id: a, p_report_id: reportId })) as unknown as Omit<ReportDetail, "evidence_photos">;
...
export async function adminTriageReport(reportId: string, opts: { priority?: Enums["report_priority"]; assigneeId?: string } = {}): Promise<Record<string, Json>> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_triage_report", { p_actor_id: a, p_report_id: reportId, p_priority: opts.priority ?? null, p_assignee_id: opts.assigneeId ?? null })) as Record<string, Json>;
}
...
export async function adminResolveReport(reportId: string, outcome: ResolveOutcome, opts: { sanctionLevel?: number; note?: string; durationHours?: number } = {}): Promise<ResolveReportResult> {
  ...
  return (await moderationRpc(createAdminClient(), "admin_resolve_report", {
    p_actor_id: a,
  ...
  })) as unknown as ResolveReportResult;
}

export async function adminReviewPhoto(photoId: string, decision: "approved" | "rejected", opts: { rejectCode?: Enums["photo_reject_code"]; note?: string } = {}): Promise<Record<string, Json>> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_review_photo", { p_actor_id: a, p_photo_id: photoId, p_decision: decision, p_reject_code: opts.rejectCode ?? null, p_note: opts.note ?? null })) as Record<string, Json>;
}

export async function adminSearchProfiles(q: string, limit = 20): Promise<Array<Record<string, Json>>> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_search_profiles", { p_actor_id: a, p_q: q, p_limit: limit })) as Array<Record<string, Json>>;
}
...
export async function adminProfileDetail(profileId: string): Promise<Record<string, Json> & { photo_urls: Record<string, string | null> }> {
  const a = await actor();
  const admin = createAdminClient();
  const raw = (await moderationRpc(admin, "admin_profile_detail", { p_actor_id: a, p_profile_id: profileId })) as Record<string, Json>;
...
export async function adminLiftSanction(sanctionId: string, note?: string): Promise<Record<string, Json>> {
  const a = await actor();
  return (await moderationRpc(createAdminClient(), "admin_lift_sanction", { p_actor_id: a, p_sanction_id: sanctionId, p_note: note ?? null })) as Record<string, Json>;
}
...
export async function adminDecideAppeal(appealId: string, decision: "accepted" | "rejected", note?: string): Promise<Record<string, Json>> {
  ...
  return (await moderationRpc(createAdminClient(), "admin_decide_appeal", { p_actor_id: a, p_appeal_id: appealId, p_decision: decision, p_note: note ?? null })) as Record<string, Json>;
}

export async function adminSetLegalHold(reportId: string, hold: boolean, note?: string): Promise<Record<string, Json>> {
  const a = await actor("admin");
  return (await moderationRpc(createAdminClient(), "admin_set_legal_hold", { p_actor_id: a, p_report_id: reportId, p_hold: hold, p_note: note ?? null })) as Record<string, Json>;
}
...
export async function runModerationDaily(): Promise<Record<string, Json>> {
  return (await moderationRpc(createAdminClient(), "moderation_daily", {})) as Record<string, Json>;
}
export async function runSlaCheck(): Promise<Record<string, Json>> {
  return (await moderationRpc(createAdminClient(), "sla_check", {})) as Record<string, Json>;
}
