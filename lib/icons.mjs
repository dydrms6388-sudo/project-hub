...
const ICONS = {
  ...
  // ── 금융 세부(같은 '건물' 아이콘 반복 방지: 도구별 구분) ──
  won: P('<circle cx="12" cy="12" r="9"/><path d="M8 9l2 6 2-4 2 4 2-6"/><path d="M7.5 12.5h9"/>'), // 연봉·급여
  gauge: P('<path d="M4.5 15a7.5 7.5 0 0 1 15 0"/><path d="M12 15l4-2.5"/><circle cx="12" cy="15" r="1.3"/>'), // DSR 한도 비율
  swap: P('<path d="M4 8.5h13"/><path d="M14 5.5l3 3-3 3"/><path d="M20 15.5H7"/><path d="M10 12.5l-3 3 3 3"/>'), // 대출 갈아타기
  receipt: P('<path d="M6 3h12v18l-2.5-1.6L13 21l-2.5-1.6L8 21l-2.5-1.6L6 21z" transform="translate(0 -1)"/><path d="M9 8h6M9 12h6"/>'), // 양도세 등 세금
  key: P('<circle cx="8.5" cy="8.5" r="4"/><path d="M11.5 11.5l7 7"/><path d="M15.5 15.5l2-2"/><path d="M17.5 17.5l2-2"/>'), // 전세대출
  rank: P('<path d="M4 20h16"/><path d="M6 20v-5"/><path d="M12 20V6"/><path d="M18 20v-9"/>'), // 연봉 순위
  // ── 건강 세부(같은 '맥박' 아이콘 반복 방지) ──
  body: P('<circle cx="12" cy="4.5" r="2.2"/><path d="M12 7v7"/><path d="M8.5 10.5l3.5-1 3.5 1"/><path d="M9 21l3-6.5 3 6.5"/>'), // 체성분·바디스캔(사람 형상)
  target: P('<circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="4.3"/><circle cx="12" cy="12" r="1" fill="currentColor" stroke="none"/>'), // 감량 목표·로드맵
  steps: P('<ellipse cx="8" cy="7" rx="2.4" ry="3.3"/><path d="M5.8 11.5c0 1.4.6 2.4 2.2 2.4"/><ellipse cx="16" cy="13" rx="2.4" ry="3.3"/><path d="M13.8 17.5c0 1.4.6 2.4 2.2 2.4"/>'), // 걸음 수·만보(발자국)
};

// 특정 slug은 카테고리 아이콘 대신 전용 아이콘으로 구분(금융 도구가 전부 같은 건물 아이콘이던 문제 해소).
const SLUG_ICON = {
  salary: 'won', 'salary-rank': 'rank', dsr: 'gauge', refinance: 'swap',
  yangdo: 'receipt', 'jeonse-loan': 'key', 'gift-money': 'won',
  age: 'calc', dday: 'calc', pyeong: 'calc', bmi: 'health',
  'body-fat-check': 'body', 'weight-roadmap': 'target', 'manbo-steps': 'steps',
};
...
// slug 전용 아이콘이 있으면 우선, 없으면 카테고리 기반.
export function iconForSlug(slug, label) {
  const k = SLUG_ICON[slug];
  return (k && ICONS[k]) ? ICONS[k] : ICONS[iconKeyFor(label)];
}
