...
- **기본 러너 = 클로드(Claude, 기본 모델 Opus)** — `.claude/settings.json`의 `"model": "opus"`가 정본. GLM 러너(`factory-glm`)는 기본이 아니며 필요 시 보조로만 연동. **과부하/호스트·Vercel 한도 감지 시 배치 크기 자동 축소(백오프)** — 멈추지 말고 줄여서 계속.
