...
"무엇을 살지"를 대신 정해주는 서비스와 정면으로 구분되는 **A안 = 추천 없는 데이터 도구** 포지션을 취한다(추천·매매 지시 표현은 UI 전면 금지).
...
| Q8 | `.env.example`이 참조하는 `pipeline/README.md`와 `pipeline/` 디렉터리가 아직 없음(P1 파이프라인 착수 시 작성). `docs/00-legal-expression-guide.md`는 P0 산출물로 존재 — `site.ts`·`check-expressions.mjs`와 문구 동기화 확인 필요 | 데이터/기획 | P1 착수 |
