...
## 5차 — hipideasv3 4종 배치 판정 (아직 안 건드린 축들)
원문 4종(Next.js/Supabase 멀티에이전트 프롬프트)을 정적 스택 기준으로 판정.
| 원문 | 판정 | 사유 |
|---|---|---|
| 읽어주는 사람 (분산 오디오북) | 보류 | 음성 업로드·스토리지·품질 게이트·삭제 파이프라인 = 백엔드 필수 |
| **둘 (2인 문답)** | **채택 → `dul`** | URL 릴레이로 정적 적응 가능. RLS 게이팅 → 클라이언트 봉인(veil)+상호 제출 시 공개로 대체 |
| 도시 하나 (공동 도시) | 보류 | 공유 영속 격자·투표·타임랩스 = DB 필수 |
| 선구안 (성장 타임스탬프) | 보류 | 주기 지표 수집(cron·플랫폼 API)·봉인 검증 = 백엔드 필수 |

### dul (둘만의 문답) — 정적 적응 설계
- 원문 "둘 다 답해야 공개"(DB RLS) → **초대 링크(st=1, 답 봉인) → 상대 제출 시 공개(st=2) → 답장 링크로 역방향 공개**. 서버 0.
- 원문 "절대 넣지 말 것"(스트릭·읽음표시·재촉·통계) 준수, 건너뛰기(이유 안 묻기)·무거운 질문 제외 토글 구현.
- 질문 은행 4트랙(부모님 32·친구 32·연인 32·형제자매 30, heavy 플래그) + 직접 질문. 날짜 결정형 선택.
- 바이럴 5요소: ?r= 상태 / 나도 해보기 CTA / canvas PNG 카드+워터마크 / 링크·텍스트·X·share / 리믹스(같은 상대·새 질문).
- 아카이브는 localStorage(기기 내). 한계 고지: 링크 소지자는 열람 가능(완전 잠금 아님) — FAQ에 정직하게 명시.
- 보류분(연말 책 POD·웹푸시·계정·RLS)은 백엔드 승인 시 재검토.

### dul 만족도 루프 (유저 10명 · 게이트 90) — **통과 91점**
| 라운드 | 평균 | 주요 지적 → 반영 |
|---|---|---|
| 1 | 76.7 | 새로고침 시 답장 링크 버튼 소실(치명)·자기 링크 열면 응답 화면·건너뛰기 무확인·기록 삭제 불가·복사 폴백 없음·PNG 하단 넘침 |
| 2 | 85.9 | sticky 배너가 결과의 질문을 덮음·배너 2줄 깨짐/닫기 불가 |
| 3 | **91** | 제출 후 배너 상태 불일치 → 갱신. 잔여: veil은 가림막 수준(FAQ 고지) |

검증: Playwright 2-컨텍스트 E2E(A/B 기기 분리)로 봉인→초대→응답→상호공개→답장링크
역방향까지 전 구간 통과(pageerror 0), 봉인 중 상대 답 DOM 미노출, 극단 입력에서 PNG
워터마크 영역 침범 픽셀 0·가로 오버플로우 0. 점수는 `data/review-scores.json`.
