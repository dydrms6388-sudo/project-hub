...
## 5차 — 기록·생활 4종 (12차 기획: 하늘 / 돈 / 음식 / 관계)
원문은 Next.js + Supabase + 웹푸시 전제. 정적 스택 적응 근거는 **`DECISIONS.md`** 참고.

| slug | 이름 | 엔진 | 저장 | 바이럴 | 상태 |
|---|---|---|---|---|---|
| sunset-forecast | 오늘 노을 | 태양위치 계산 + 가중치 테이블 | localStorage | ✅ | 완료 |
| cost-per-wear | 1회당 | 원가 나눗셈 + 편중도 통계 | localStorage | ✅ | 완료 |
| recipe-tree | 레시피 계보 | 규칙 파서 + 구조 diff | localStorage + URL | ✅ | 완료 |
| sinse | 신세 | 집계만 (판정 없음) | localStorage | ✅ | 완료 |

핵심 적응(전부 앱 화면에 명시):
- **노을**: 기상청/에어코리아 API는 키 필수라 정적에서 불가, 키 없는 무료 API는 비상업
  한정이라 AdSense 사이트에 부적합 → 구름·대기는 **사용자 입력**, 일몰·방위·박명은
  **순수 계산**(KASI 공표값과 ±1분 검증). LLM 호출 0건, 가중치는 설정 테이블 1곳에서만.
  웹푸시 → **.ics 캘린더 알림(일몰 90분 전)**. 적중률은 로컬 기록 5건부터, 낮아도 그대로 표시.
- **1회당**: 신체 관련 필드 0개, 제휴·외부 링크 0건(감사 완료). 사진은 localStorage 용량
  한계로 이모지 타일 대체. 구독은 `월 요금 ÷ 그 달 사용 횟수`.
- **레시피 계보**: LLM 파싱 → 정규식+단위사전 규칙 파서 + 사용자 확인 필수. 포크 시 부모
  스냅샷 동봉(부모 삭제돼도 diff 생존). 남의 포크 수는 알 수 없으므로 "내 기기 기준"으로 표기.
- **신세**: `Notification`/`serviceWorker`/연락처 API 호출 0건. 점수·등급·적자흑자 0건.
  앱 잠금은 정적에서 실질 보안이 안 되므로 **의도적 미구현**(사유 화면 명시).

검증: `node scripts/test-recipe-diff.mjs` **48/48 통과**(배포 코드를 그대로 추출해 테스트),
`node gen-pages.mjs` 경고 0(내장 26→30), 4앱 헤드리스 렌더 확인, JSON-LD 유효.

## 소유자 결정 필요 (과금/키)
- 서버사이드 AI 프록시 도입(진짜 AI 결과 품질) → Supabase/AI 키·비용 승인 필요.
- 그 경우에만 tone-lab/roast-edit/first-impress 등의 AI 버전, daily-debate cron 가능.
- 5차 관련: 노을 기상 API 실연동·웹푸시·지역 랭킹, 1회당 기기 간 동기화·사진,
  레시피 개별 SSR + Recipe JSON-LD(장기 SEO 자산). 상세는 `DECISIONS.md` 마지막 절.
