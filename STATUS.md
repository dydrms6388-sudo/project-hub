...
| 원문 | 판정 | 사유 |
|---|---|---|
| 1. 사라진 것들의 아카이브("없다") | 보류 | Supabase(PostGIS)+fal.ai 이미지 생성+검수 큐 = 백엔드·유료 키 필수 |
| 2. 튜링 살롱 | 보류 | 실시간 WS 멀티플레이 + Claude API 상시 호출 = 백엔드·유료 키 필수. 원문 추천 1순위지만 정적 스택에선 성립 불가 |
| 3. 11시 11분 | 보류 | 서버 시간 강제·제출 큐·웹푸시·조판 배치 전부 서버 필요 |
| **4. 사이(Latent Walk)** | **채택** | 어휘 임베딩을 빌드타임에 사전 구축하면 완전 클라이언트로 성립 |
