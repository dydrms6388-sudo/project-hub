...
## 5차 — AI 작사작곡 스튜디오 `/songlab/`
| 구성 | 구현 | 비고 |
|---|---|---|
| 감성 무드 8종 | 무드별 코드 진행·리듬·리프트·훅 프리셋 | 설렘/이별/새벽/응원/시티팝/발라드/댄스/로파이 |
| 작곡 엔진 | 2마디 모티프 생성 → 마디별 화음 스냅·변형 전개 | 벌스·프리·코러스·브릿지, 12키 × 장단조 |
| 작사 엔진 | 감성 계열 4종 문장뱅크 + 음절 수 맞춤 | 멜로디 음표 수 = 가사 음절 수(matchCount) |
| 허밍 → 악보 | 자기상관(NSDF) 음정 추적 + 첫 봉우리 선택 | 옥타브 오인식 방지, 진폭 온셋 분리 |
| 조성·코드 추출 | Krumhansl 프로파일 상관 + 마디별 DP 화음 배정 | 마이크·음원 파일 양쪽 지원 |
| 악보 렌더 | SVG 오선(조표·음자리표·임시표·덧줄·부점·꼬리) | 컨테이너 폭에 맞춰 마디 수 자동 조절 |
| 재생 | Web Audio 멜로디+코드+베이스(+드럼) | 마디 위치 표시 |
| 내보내기 | 표준 MIDI(4트랙) · 결과 카드 PNG · 가사/코드 텍스트 | |
| 바이럴 5요소 | `?r=` 결과 링크(허밍 멜로디까지 포함) · 유입 CTA · 카드 · 공유 4종 · 리믹스 | ✅ |

검증: 합성 허밍(도레미도 미파솔 파미레도, 100BPM)으로 음정 11/11 정확,
조성 C 장조·템포 100BPM·코드 `C | G7 | F | C` 추출. 실제 브라우저(마이크 가짜
입력·파일 업로드) 양 경로 및 재생 출력 신호까지 확인. 외부 AI API·서버 없음.
