...
## 5차 — "힙한 것들 4종" 기획문서 판정 (정적 스택 기준)
| 원문 | 판정 | 사유 |
|---|---|---|
| 1. 사라진 것들의 아카이브("없다") | **프런트 데모**(`/eopda`) | 본편은 Supabase(PostGIS)+fal.ai 이미지 생성+검수 큐 = 백엔드·유료 키 필수 |
| 2. 튜링 살롱 | **프런트 데모**(`/turing-salon`) | 본편은 실시간 WS 멀티 + Claude API 상시 호출 필수. 원문 추천 1순위지만 정적으론 성립 불가 |
| 3. 11시 11분 | **프런트 데모**(`/eleven11`) | 본편은 서버 시간 강제·제출 큐·웹푸시·조판 배치 필요 |
| **4. 사이(Latent Walk)** | **채택(완품)** | 어휘 임베딩을 빌드타임에 사전 구축하면 완전 클라이언트로 성립 |

### 실험실 프런트 데모 3종 (허브 "🧪 실험실" 카테고리, noindex)
소유자 요청("우선 프런트라도 1 2 3")으로 제작. 전부 서버 없는 체험판이며
페이지 상단에 데모 한계를 명시. 백엔드 승인 시 본편의 프런트 초안으로 재사용.
| slug | 데모에서 되는 것 | 본편에서만 되는 것(보류 사유) |
|---|---|---|
| eopda | 가상 동네 '온기동' SVG 지도·연도별 핀, 감각 프롬프트 4문항(2+ 제출), 절차적 회화풍 상상도(+워터마크·기억 수에 따라 재렌더·버전 타임라인), PII/욕설 차단, localStorage, 공유 카드/링크 | 실제 지도(카카오맵), AI 상상도(fal.ai), 다인 증언 누적, 검수 큐·신고 |
| turing-salon | 싱글플레이: 각본 봇 5명(2~3명이 '안티패턴' AI 말투 연기), 타이핑 인디케이터·지연·오타 연출, 지목/점수(+2/-1/인간판별+3), 판별 정확도·"N명이 나를 AI로", 누적 통계, 공유 카드, 유해발화 필터 | 실시간 멀티(WS), 진짜 Claude 에이전트, 방 상태머신·자동 충원 |
| eleven11 | KST 23:11 60초 게이트(기기 시계·데모 명시), 111자·수정불가, 규칙기반 필터, 연습 모드, 로컬 아카이브+시드 예시, 글 수 적응형 조판 이미지, .ics 알림 | 서버 시간 강제·동시 제출 큐, 전체 참여자 조판, 웹푸시, LLM 2차 검수 |

### 사이(/sai) 구현 노트
- 어휘 인덱스: fastText `cc.ko.300`(공개 데이터, 빈도순) 상위 25만 토큰 →
  조사·어미 파생형/불용어/혐오·비하어/실존 인물 필터 → **30,010 단어**.
  빌드 스크립트 `scripts/sai-build-vectors.py` (재현 커맨드 주석 참고).
- 원문 pgvector+HNSW 대체: 300d → PCA 128d(설명분산 0.60) → int8 양자화 →
  `sai/data/vecs.bin` 3.8MB + `words.json` 0.35MB, 브라우저에서 전수 내적 탐색
  (30k×128d ≈ 수 ms, 서버 없이 지연 목표 충족).
- slerp 보간 + 최근접 탐색은 원문 요구대로 **실제 벡터 공간 탐색**(LLM 생성 아님).
  결정적 → 공유 URL(`?r=`) 완전 재현. VIRAL 5요소(URL 상태·나도 해보기·캔버스
  카드+워터마크·공유 4종·리믹스) 구현. 입력은 사전 내 단어로 제한(자동완성).
- 원문 대비 미구현: 단계별 이미지 생성(유료 키), 음악 모드(스트리밍 API) — 보류란과 동일 사유.

## 소유자 결정 필요 (과금/키)
- 서버사이드 AI 프록시 도입(진짜 AI 결과 품질) → Supabase/AI 키·비용 승인 필요.
- 그 경우에만 tone-lab/roast-edit/first-impress 등의 AI 버전, daily-debate cron 가능.
- "힙한 것들" 1·2·3번(없다/튜링 살롱/11시 11분)도 같은 승인이 있어야 착수 가능 —
  전부 별도 Next.js/Supabase 프로젝트로 시작해야 하며 이 정적 저장소에는 얹을 수 없음.
