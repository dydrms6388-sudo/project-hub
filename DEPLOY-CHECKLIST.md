...
7. **`auth-hub` 결제 백엔드 — 심각 취약점 4건 수정 완료**(크레딧 이중지불→원자적 조건부 차감 · IAP 권한상승→검증 상품 기준 엔타이틀먼트 · 모바일토큰 오픈리다이렉트→화이트리스트 미설정 시 deny-all · Toss 웹훅 무인증→미설정 시 거부+상수시간 비교). 적대적 재검증 통과(보안 92 / 무회귀 93). **여전히 미배포·미연동**이며 배포하려면: (a) `auth-hub/.env.example`의 모든 시크릿 설정, (b) **`ALLOWED_MOBILE_SCHEMES` 반드시 설정**(미설정 시 네이티브 로그인 리다이렉트 전부 거부), (c) **`TOSS_WEBHOOK_SECRET` 반드시 설정**(미설정 시 웹훅 503), (d) `prisma migrate deploy`로 DB 스키마 적용, (e) `npm run build`(=`prisma generate && next build`)로 타입/빌드 확인(이 환경엔 deps 미설치라 로컬 빌드 미검증). 정적 프론트는 이 백엔드를 호출하지 않는다.
   - **잔여 하드닝(비차단, 별도 권장)**: `mobile/issue` 화이트리스트는 스킴 단위라 운영자가 `https`를 화이트리스트에 넣으면 오픈리다이렉트가 재개방됨(커스텀 앱 스킴만 넣을 것) · Stripe `checkout.session.completed`의 `payment_status==="paid"` 미검증(비동기 결제 정산 레이스) · IAP 크로스유저 first-claim(영수증이 허브 계정에 바인딩 안 됨).
